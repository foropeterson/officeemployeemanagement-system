...
def addEmp(request):
    if request.method == 'POST':
        ...
        dept_id = int(request.POST['dept'])  # Adjusted to use dept_id instead of department_id
        role_id = int(request.POST['role'])

        # Use get_object_or_404 with the Department model to retrieve the department
